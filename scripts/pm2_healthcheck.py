"""pm2_healthcheck.py — Check all Nestaro Pilot tier Nestaro Pilot PM2 processes and restart stopped ones.

Nestaro Pilot tier runs 8 PM2 processes (vs earlier prototype's 5):
  earlier prototype: filesystem-watcher, gmail-watcher, whatsapp-watcher, orchestrator, email-mcp
  Nestaro Pilot adds: facebook-watcher, instagram-watcher, twitter-watcher
...
# Expected process names for Nestaro Pilot tier (must match pm2_ecosystem.config.js)
EXPECTED_PROCESSES = {
    "filesystem-watcher",
    "gmail-watcher",
    "whatsapp-watcher",
...
def _print_health_table(process_map: dict[str, str], restarted: list[str], missing: list[str]) -> None:
    ...
    print("\n" + "=" * 58)
    print("  Nestaro Pilot Nestaro Pilot — PM2 Health Status")
...
def run_healthcheck(dry_run: bool = False) -> None:
    ...
    logger.info("Starting PM2 health check (Nestaro Pilot tier, 8 processes) (dry_run=%s)", dry_run)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PM2 health check for Nestaro Pilot Nestaro Pilot tier processes (8 processes)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
