...
Monitors vault/Inbox for new files using watchdog, creates metadata .md files
in vault/Needs_Action with YAML frontmatter per data-model.md VaultItem schema.
"""
...
class FilesystemWatcher(BaseWatcher):
    ...
    def _is_duplicate(self, original_name: str) -> bool:
        """Check if metadata file already exists in Needs_Action."""
...
"""
    ...
    def _startup_scan(self) -> None:
        """Scan Inbox for unprocessed files on startup.
    ...
    def run(self) -> None:
        ...
        - Startup scan before entering loop
        - Watchdog Observer for real-time file detection
        """
        self._running = True

        # Process files that arrived while watcher was stopped
        self._startup_scan()
...
def main() -> None:
    ...
    watcher = FilesystemWatcher(vault_path=vault_path, check_interval=check_interval)
