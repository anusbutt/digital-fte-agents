...
### T042 — `./watchers/gmail_watcher.py` auth error handling
...
### T043 — `./watchers/facebook_watcher.py` auth error handling
...
### T044 — `./watchers/instagram_watcher.py` auth error handling
...
### T045 — `./watchers/twitter_watcher.py` auth error handling
...
### T046 — `./watchers/orchestrator.py` approval expiry enforcement
...
### T047 — `./scripts/validate_dry_run.py` smoke test

9-check end-to-end smoke test. Forces `os.environ["DRY_RUN"] = "true"` before
running. Uses `./vault/` by default (configurable via `--vault`).
