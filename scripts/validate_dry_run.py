"""validate_dry_run.py — End-to-end smoke test for Nestaro Pilot tier with DRY_RUN=true.
...
def run_smoke_test(test_vault: Path, verbose: bool) -> bool:
    ...
    print(f"\n{'='*60}")
    print("  Nestaro Pilot Tier DRY_RUN Smoke Test")
    ...
    if failed == 0:
        print("\n[+] ALL CHECKS PASSED -- Nestaro Pilot tier DRY_RUN smoke test OK\n")
        return True
    else:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Nestaro Pilot tier DRY_RUN smoke test — validates pipeline without external calls"
    )
    parser.add_argument(
        "--vault",
