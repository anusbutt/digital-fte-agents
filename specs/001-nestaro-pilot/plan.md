# Implementation Plan: Nestaro Pilot Nestaro Pilot — Autonomous AI employee
...
Build the Nestaro Pilot tier of the Nestaro Pilot system — a fully autonomous AI employee
that extends earlier prototype with: three new social media channels (Facebook, Instagram,
...
data, comprehensive error recovery, and full JSON audit logging. Nestaro Pilot is
self-contained under `./` with no runtime dependencies on earlier prototype or earlier prototype.
...
**Project Type**: Single project (self-contained under `./`)
...
for social media (no paid APIs); local-first (all data on-machine); Nestaro Pilot
fully self-contained from earlier prototype/earlier prototype
...
│   ├── filesystem_watcher.py   # Monitors vault/Inbox/ (from earlier prototype)
│   ├── gmail_watcher.py        # Polls Gmail API (from earlier prototype)
│   ├── whatsapp_watcher.py     # Playwright WhatsApp Web (from earlier prototype)
...
│   └── orchestrator.py         # Master controller — expanded for Nestaro Pilot (from earlier prototype+)
│
├── skills/
│   ├── triage-inbox.md                 # Classify + plan all item types (expanded)
│   ├── compose-email-reply.md          # Draft/send email replies (from earlier prototype)
│   ├── compose-whatsapp-reply.md       # Draft WhatsApp replies (from earlier prototype)
│   ├── generate-linkedin-post.md       # LinkedIn post draft (from earlier prototype)
...
│
├── email-mcp/                   # Gmail send/draft MCP server (from earlier prototype)
│   ├── index.ts
│   └── package.json
│
...
│
├── scripts/
│   ├── linkedin_post.py         # LinkedIn Playwright post (from earlier prototype)
...
**Structure Decision**: Single project under `./`. All watchers, skills,
...
*See `specs/001-nestaro-pilot/research.md` for full findings.*
...
- PM2 ecosystem expanded from 5 (earlier prototype) to 8 processes
- `generate-accounting-briefing` skill replaces `generate-briefing` for Nestaro Pilot
